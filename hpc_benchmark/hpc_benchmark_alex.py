...
params = {
    ...
    'compressed_spikes': False,  # whether to use spike compression
...
interrupt = False # Run with interruptions

NE = 9000
NI = 2250

brunel_params = {
    'with_stdp': False, # switch on STDP
    'neuron_model': 'iaf_psc_exp', # 'iaf_psc_alpha' also possible
    'NE': int(NE * params['scale']),  # number of excitatory neurons
    'NI': int(NI * params['scale']),  # number of inhibitory neurons
    'CE' : min(1000, int(NE * params['scale'])), # number of incoming excitatory connection
    'CI' : min(1000, int(NI * params['scale'])), # number of incoming inhibitory connection
    ...
    'eta': 2.6,  # scaling of external stimulus
...
def build_network():
    ...
    neuron_model = brunel_params['neuron_model']
    CE = brunel_params['CE']
    CI = brunel_params['CI']
    with_stdp = brunel_params['with_stdp']

    if neuron_model == 'iaf_psc_exp':
        convert_synapse_weight = convert_synapses_weight_exp
    elif neuron_model == 'iaf_psc_alpha':
        convert_synapse_weight = convert_synapse_weight_alpha
    ...
    E_neurons = nest.Create(neuron_model, NE, params=model_params)

    nest.message(M_INFO, 'build_network', 'Creating inhibitory population.')
    I_neurons = nest.Create(neuron_model, NI, params=model_params)
    ...
    if with_stdp:
        if params['record_spikes']:
            ...
            recorder = nest.Create('spike_recorder', params={
                'record_to': 'ascii',
                'label': recorder_label
            })
    else:
        if params['record_spikes']:
            recorder_label_E = os.path.join(
                brunel_params['filestem'],
                'spikes-E-' + brunel_params['neuron_model'])
            recorder_label_I = os.path.join(
                brunel_params['filestem'],
                'spikes-I-' + brunel_params['neuron_model'])
            recorder_E = nest.Create('spike_recorder', params={
                'record_to': 'memory',
                'label': recorder_label_E
                })
            recorder_I = nest.Create('spike_recorder', params={
                'record_to': 'memory',
    ...
    if params['record_spikes']:
        ...
        nest.Connect(local_neurons_E, recorder_E, 'all_to_all', 'static_synapse_hpc')
        nest.Connect(local_neurons_I, recorder_I, 'all_to_all', 'static_synapse_hpc')
    ...
    recorders = (recorder_E, recorder_I) if params['record_spikes'] else None

    neurons = (E_neurons, I_neurons)

    return d, recorders, neurons


def run_simulation(interrupt, num_neurons, indegree):
    ...
    brunel_params['CE'] = indegree
    brunel_params['CI'] = indegree
...
def compute_rate(sr):
    ...
    n_local_spikes_E = sr[0].n_events
    n_local_spikes_I = sr[1].n_events
